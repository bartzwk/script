...
OUTPUT_XLS = os.path.join(ROOT_DIR, "master FA1 assembled.xls")
SEARCH_TOP_FOLDERS = ["A1", "A2", "B", "C1", "C2"]
...
def should_skip_file(full_path: str, root_dir: str) -> Tuple[bool, str]:
    ...
    if parts[0].upper() not in [f.upper() for f in SEARCH_TOP_FOLDERS]:
...
def parse_number_fields(number: object, zone_from_path: str = "", company_from_meta: str = "", revision_from_meta: object = None) -> Dict[str, object]:
    ...
    s = re.sub(r"\s+", " ", s)
    # Strip FA1/FA project prefix if present
    s = re.sub(r"^FA\d*[-_ ]*", "", s, flags=re.IGNORECASE)
...
def main() -> None:
    ...
    if not files:
        print("[ERROR] No se han encontrado archivos válidos en A1/A2/B/C1/C2.")
        return
